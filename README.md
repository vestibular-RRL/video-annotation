...
A comprehensive Python-based video annotation tool for frame-by-frame annotation with CSV export functionality. Built with PyQt6 and OpenCV, this tool provides an intuitive interface for annotating video frames with various shapes and exporting the annotations to CSV format. This simplified version focuses on video playback and manual annotation without object tracking capabilities.
...
- **Export with Trimmed Video**: Export CSV with automatically trimmed video clip
...
3. **Set Annotation Range**: Use the annotation range sliders to set start/end times
4. **Apply Annotations**: Enter annotation text and click "Apply to Range"
5. **Export Options**: 
   - Use File → Export CSV for basic CSV export
   - Use File → Export CSV with Trimmed Video to create a folder with video clip and CSV
...
#### Export with Trimmed Video
Creates a dedicated folder containing:
- **Trimmed Video**: Video clip from the selected frame range
- **CSV File**: Annotations for the selected frame range only
- **Summary File**: Export information including frame range, duration, and statistics

This feature automatically:
- Creates a folder named after the video and frame range
- Trims the video to the specified frame range
- Exports only the annotations for that range
- Generates a summary file with export details
...
│   │   ├── video_trimmer.py      # Video trimming
